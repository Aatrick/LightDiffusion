...
def cast_to_device(tensor, device, dtype, copy=False):
    if copy:
        return tensor.to(device, copy=copy, non_blocking=True).to(
            dtype, non_blocking=True
        )
    else:
...
def prepare_sampling(model, noise_shape, conds):
    ...
    return real_model, conds


def cast_bias_weight(s, input):
    bias = None
    if s.bias is not None:
        bias = s.bias.to(device=input.device, dtype=input.dtype, non_blocking=True)
    weight = s.weight.to(device=input.device, dtype=input.dtype, non_blocking=True)
    return weight, bias
...
class CFGGuider:
    ...
    def sample(
        ...
        self.inner_model, self.conds = prepare_sampling(
...
class SDTokenizer:
    def __init__(
        ...
        self.embedding_directory = "_internal\\embeddings"
...
def model_config_from_unet_config(unet_config, state_dict=None):
    if sm_SD15.matches(unet_config, state_dict):
        return sm_SD15(unet_config)
